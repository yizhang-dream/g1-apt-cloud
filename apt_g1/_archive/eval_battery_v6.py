"""Full battery v6 + oracle stealth check."""
import json, os, sys
import numpy as np
import torch
...
odir = '/home/cvgluser/ros2_data/apt_g1/outputs/distill_v6'
...
for name, m, sp, a in scen:
    ...
    if gi is None:
        print(f'{name:12s} NO GROUP', flush=True); out[name] = {'note': 'no router'}; continue
    ...
    for seed in [0, 1, 2]:
        ...
        print(f'{name:12s} seed{seed} done={r["completed"]} h_min={r["h_min"]} vx={r["vx"]} vy={r["vy"]} path={r["path"]}', flush=True)
json.dump(out, open(f'{odir}/eval_battery_v6.json', 'w'), indent=1)

# oracle stealth replay
rows = np.where((mode == 18) & (np.abs(speed + 1) < 1e-6) & (ab == 3))[0]
print('stealth oracle rows', len(rows), flush=True)
env.reset(); vxs=[]; hs=[]; fall=None
for t in range(500):
    obs, r, term, info = env.step({'token': token[rows[t % len(rows)]], 'aux': np.zeros(12, dtype=np.float32)})
    v = env._get_base_linear_velocity(); vxs.append(float(v[0])); hs.append(float(env.data.qpos[2]))
    if term:
        fall = t; break
print('stealth oracle:', dict(fall=fall, vx=round(float(np.mean(vxs)),3), h_min=round(float(min(hs)),3)), flush=True)
...
def run_switch(sched, seed=4):
    ...
    for name, secs in sched:
        ...
        for t in range(int(secs * 50)):
            ...
            phi = float(np.arctan2(sc[0], sc[1]))
            b = int(np.floor((phi + np.pi) / (2 * np.pi) * B) % B)
            obs, reward, terminated, info = env.step({'token': protos[gi][b], 'aux': np.zeros(12, dtype=np.float32)})
            v = env._get_base_linear_velocity()
            vxs.append(float(v[0])); vys.append(float(v[1])); heights.append(float(env.data.qpos[2]))
            if terminated:
...
json.dump({'switch': sw, 'schedule': sched}, open(f'{odir}/eval_switch_v6.json', 'w'), indent=1)
print('done')