"""Phase-router final battery: B=40 mean prototypes + optional phase EMA; 1000 steps x 3 seeds + switch."""
import json, os, sys
import numpy as np
import torch
...
# quick EMA sweep on the marginal modes
for ema in [0.0, 0.3, 0.6]:
    for name in ['slow_walk', 'walk', 'jump']:
        c = dict(scen[[n for n, _ in scen].index(name)][1])
        gi = gmap[name]
        comp = 0
        mins = []
        for seed in [0, 1, 2]:
            r = run(c, gi, 1000, seed, ema)
            comp += 1 if r['completed'] else 0
            mins.append(r['h_min'])
        print(f'ema={ema} {name:10s} completed {comp}/3 h_min_avg {np.mean(mins):.3f}', flush=True)

# definitive battery with best ema
best_ema = 0.3
out = {}
for name, c in scen:
    gi = gmap[name]
    out[name] = {}
    for seed in [0, 1, 2]:
        r = run(c, gi, 1000, seed, best_ema)
        out[name][f'seed{seed}'] = r
        print(f'FINAL {name:10s} seed{seed} completed={r["completed"]} h_min={r["h_min"]} vx={r["vx"]} disp={r["disp"]}', flush=True)
