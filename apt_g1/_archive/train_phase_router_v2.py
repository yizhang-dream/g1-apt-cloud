"""Build merged dataset (exp1+exp2) and train phase-regression routers per (mode, speed, dir-angle-bin) group."""
...
def build(pi_path, cmd_path, tok_path):
    ...
    assert np.abs(PI[:, :64] - tok[:, 5:69]).max() < 1e-6
    N, M = len(PI), len(CMD)
...
out = '/home/cvgluser/ros2_data/apt_g1/data/exp_all'
os.makedirs(out, exist_ok=True)
np.save(out + '/proprio.npy', proprio)
np.save(out + '/cmd.npy', cmd)
np.save(out + '/token.npy', token)
np.save(out + '/mode.npy', mode)
np.save(out + '/speed.npy', speed)
np.save(out + '/angle_bin.npy', angle_bin)
np.save(out + '/meta_modes.npy', modes)
print('saved', out, 'modes', [int(m) for m in modes])
...
print('group sizes:')
for g, v in sorted(groups.items(), key=lambda x: -len(x[1]))[:25]:
    print('  ', g, len(v))
...
for gi, (g, rows) in enumerate(groups.items()):
    ...
    nets[gi] = net
    protos[gi] = proto
    meta[str(gi)] = dict(group=list(g), rows=len(rows), mu=mu.tolist(), V2=V2.tolist(), n_bins=B)
    torch.save(net.state_dict(), f'{odir}/phase_g{gi}.pt')
    if gi % 3 == 0:
        print(f'group {g} rows {len(rows)} ang_err {best:.4f} ({round(time.time()-t0,1)}s)', flush=True)
...
json.dump({'groups': {str(gi): list(g) for gi, g in enumerate(groups.items())}}, open(f'{odir}/groups.json', 'w'))
