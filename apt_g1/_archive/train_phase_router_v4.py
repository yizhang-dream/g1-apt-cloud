"""v4: retrain slow_fwd group on exp1 rows only; other groups unchanged (reuse v3)."""
...
odir4 = '/home/cvgluser/ros2_data/apt_g1/outputs/distill_v4'
...
print('slow gi', gi_slow, 'rows was', meta3[str(gi_slow)]['rows'])

# copy v3 artifacts for all groups except slow; retrain slow on exp1 rows
rows_slow = np.where((mode == 1) & (np.abs(speed - 0.2) < 1e-6) & (ab == 4) & (np.arange(len(mode)) < 20838))[0]
print('exp1 slow rows', len(rows_slow))
T = token[rows_slow]
mu = T.mean(0)
Tc = T - mu
...
np.save(f'{odir4}/proto_g{gi_slow}.npy', proto)
...
for ep in range(60):
    ...
    if err < best:
        best = err
        torch.save(net.state_dict(), f'{odir4}/phase_g{gi_slow}.pt')
    if (ep + 1) % 10 == 0:
        print(f'ep {ep+1} loss {tl/tb:.5f} ang_err {err:.4f}', flush=True)
print('best ang_err', best)
# write meta with slow replaced + copy other artifacts
meta4 = dict(meta3)
meta4[str(gi_slow)] = dict(group=[1, 0.2, 4], rows_all=int(len(rows_slow)), rows=int(len(rows_slow)),
                           mu=mu.tolist(), V2=V2.tolist(), n_bins=B, kept=1.0, note='exp1-only')
json.dump(meta4, open(f'{odir4}/phase_meta.json', 'w'), indent=1)
for gi, md in meta3.items():
    gi = int(gi)
    if gi == gi_slow:
        continue
    import shutil
    shutil.copy(f'{odir3}/phase_g{gi}.pt', f'{odir4}/phase_g{gi}.pt')
    shutil.copy(f'{odir3}/proto_g{gi}.npy', f'{odir4}/proto_g{gi}.npy')
shutil.copy(f'{odir3}/phase_norm.npz', f'{odir4}/phase_norm.npz')
print('v4 ready')