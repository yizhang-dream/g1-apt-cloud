"""Battery v2.3: density-filtered routers, fdir=mdir for curved scenarios, 2D metrics."""
import json, os, sys
import numpy as np
import torch
...
odir = '/home/cvgluser/ros2_data/apt_g1/outputs/distill_v4'
norm = np.load(odir + '/phase_norm.npz')
meta = json.load(open(odir + '/phase_meta.json'))
pmean = norm['pmean'].ravel(); pstd = norm['pstd'].ravel(); B = 40
nets = {}; protos = {}
...
for name, m, sp, a in scen:
    ...
    c = dict(mode=m, speed=sp, mdir=mdir, fdir=mdir)  # fdir=mdir (curved motion follows facing)
    out[name] = {}
    for seed in [0, 1, 2]:
...
json.dump(out, open(f'{odir}/eval_battery_v4.json', 'w'), indent=1)

# prototype replay sanity for slow_fwd
gi = gmap[(1, 0.2, 4)]
env.reset(); vxs=[]
for b in range(400):
    obs, r, term, info = env.step({'token': protos[gi][b % 40], 'aux': np.zeros(12, dtype=np.float32)})
    v = env._get_base_linear_velocity(); vxs.append(float(v[0]))
    if term:
        print('proto replay fell at', b); break
print('slow proto replay vx:', round(float(np.mean(vxs)), 3), flush=True)
...
json.dump({'switch': sw, 'schedule': sched}, open(f'{odir}/eval_switch_v4.json', 'w'), indent=1)
print('done')