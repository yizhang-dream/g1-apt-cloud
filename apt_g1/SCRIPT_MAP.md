...
| `ppo_core.py` | MODULE | 向量化 PPO（含论文式训练附加项；E44 增加 `decoder_ft` 分支与 `decoder_reg_coef`；**E49 修复：GAE 边界 done|trunc 都切断递推且不自举 + aux_executed=False 时 aux 不进 log_prob/entropy + 真 epoch 循环 + approx_kl/clip_frac/act_std 指标，stats 键 `kl` 更名 `kl_prior`**）；**D048i 增 `kl_step_guard` 步级解析 KL 看门+资格检查（`kl_diag_gaussian_reverse`/`kl_categorical_reverse` 解析 KL(old‖new) + `ksg_*` 快照/还原/资格检查/看门步方法族，update() 内独立 `elif ksg_on` 分支，默认关=冻结版行为不变）**；**D049b 增可选 `vb_head`（`nn.Linear(hidden,3)`，gate 头完全同模式：Categorical 采样进 act()/update()/ksg_joint_logp/post_update_kl 的联合 log_prob 与熵，ksg 仅顺带记录 kl_vb 不进联合目标；旗标关=不建头零变化）**；**D049b-fix（owner 核验 P1 契约缺陷：categorical 采样索引进 log_prob 而 env 执行 softmax(logits)，E[A∇log p(k)]=0）：vb_head 路径整体重写为连续高斯（与 z/res 同构）——新增可学习 `vb_log_std`（Parameter 全 init 值填充，默认 -4 同 z/res 约定），act() 采样 `vb_action=vb_logits+exp(vb_log_std)·ε`（det=均值=vb_logits）并返回 `out["vb_action"]`，log_prob/熵改高斯密度/微分熵；update() 重算/ksg_joint_logp/post_update_kl 同改（rollout 键 `vb`→`vb_action`，连续 3 维）；ksg_eval_kl 的 kl_vb 改 `kl_diag_gaussian_reverse` 解析 KL 并**纳入 joint 目标**（joint=z+res+vb，不再仅记录），qual_kl_self 补 vb 自 KL；旧 categorical 路径删除不留旗标，旧 D049b ckpt strict 加载缺 `vb_log_std` 即明确报错（执行失败不兼容=预期）；vb_head=False 路径逐位不变**；**D051 增模块级纯函数 `vb_bc_target_w`/`vb_bc_loss`（BC 目标软权重 w*(cmd) 分段线性分配 + CE 监督，train 预热与 d049_consumer_gate G4 共用同一实现）+ update() 伴生插桩 `adv_vb_corr`（advantage×buffer vb_action 逐维 Pearson，只读不改训练，vb 头关闭=None）** |
| `train_apt_isaac.py` | 入口 | 训练 APT（相位路由器先验 + aux）策略；TO42 修订 v4 增 `--ppo-minibatch`（默认 512 = 既有行为不变；2048envs 操作点用 4096）；**E49 增 `--token-mode/--token-phase-obs/--token-alpha/--token-bound/--token-stats`（直出 64d token，无 VAE）；E49 修复轮增 `--ppo-epochs`（默认 1 = 历史单遍）+ latent/token/to42 置 `aux_executed=False` + vx 拆 fwd（机体系带符号）/spd（模长，hist `vx` 键不变）双口径 + hist 增 approx_kl/clip_frac/act_std + `policy_it_0.pt` 初始快照**；**D048h：三 ckpt 保存点（it_0/50it 阶梯/final）改 `ckpt_identity.save_ckpt` 带 ckpt_identity 配置身份块（git_head/env_sha256/rew_contract 与 train_log 同源复用，vae/decoder md5 资产加载后各算一次），`--resume` 兼容解包新格式**；**D048i：`--kl-step-guard` 步级解析 KL 看门+资格检查（与 `--kl-guard` 互斥 argparse 报错；hist 增 14 个 ksg 键仅开启时初始化；资格违约逐条打印后 exit 8、连续死轮存 `_deadround` 诊断 ckpt 后 exit 7，两路径退出前均落盘 train_log）**；**D049b-fix：buf 键 `vb`（long 索引）→`vb_action`（T,N,3 float）——act 采样/buffer/送 env 动作段（residual 48 维 45:48、plain 31 维 28:31）同一张量贯穿，不再拼 forward 原始 logits；KSG_HIST_KEYS 补 `kl_vb_mean`（kl_vb 已进 ksg joint 目标）；`--vb-from-policy` 注释/help 同步（vb 连续动作，det=vb_logits）**；**D051：`--vb-bc-warmup-steps/--vb-bc-lr/--vb-bc-anchors`——主训练循环前 BC 预热 vb_logits（独立 Adam 只动线性层，act no_grad 采样 + env.step 在线 rollout，CE(obs, cmd→w*)），预热末 obs[67] 命令网格校准表落 hist["vb_bc_calib"]/["vb_bc_info"]；hist 增 `adv_vb_corr`（旗标开时初始化）** |
| `eval_apt_isaac.py` | 入口 | A/B/C/D 评测；**E49 增 token-mode 同款旗标；E49 修复轮增 `--init-policy`（未训练初始化对照，`--checkpoint` 随之转 optional）；D048f 阶段0（metrics_contract=d048f_stage0）增：任务成功指标（vx_rmse/yaw_err_int/lat_max/fwd_max/survived_budget/task_success 冻结门）+逐局初态指纹（md5 配对核验）+每 entry 命令刷新+显式失败纪律（缺/坏 ckpt、空结果非零退出，不退回随机模型）+`--impulse-n`（term 分支探针）**；**D048h：ckpt 身份核验（新格式 ckpt 逐键比对 res_scale/res_clip/latent_residual/维度/vae·decoder md5，失配 exit 6 在跑局前拒绝；legacy WARNING 继续）+ `--res-stats`（env 侧残差执行统计逐局 res_diag 落盘 + 顶层聚合，metrics_contract=d048h_resdiag；2026-09-09：res_diag 增 `res_joint_signed_mean` 带符号逐关节键、顶层 `_agg_res_diag` 同步聚合）+ out 新增 `ckpt_identity` 块与 cfg `decoder_md5` 键**；**2026-09-09：ident_expect 补默认 apt 路径 `action_space`（14/31）——format=1 双侧必填硬化后 eval 侧 7 键不可再缺**；D048l：--force-vbin 强制速度档评测干预（eval_interventions 记录）+yaw_err_deg/heading_gate_pass 单位防误读；**D049b-fix：vb 槽位改放 act 的连续 `vb_action`（det 默认模式=vb_logits，softmax 与旧确定性 w 逐位一致=评测口径不变；--sample 模式带噪、与训练 rollout 同口径），不再拼 forward logits；vb_w_mean/vb_argmax_frac 统计照旧（env 侧未变）** |
...
| `d049_consumer_gate.py` | MODULE/test | **D049b-fix 消费者测试门（纯 torch CPU，无 isaaclab 依赖；本机 torch 2.12.0+cpu 与服务器 .venv_isaac 均可实跑）**：仓库根 `PYTHONPATH=. python apt_g1/isaac/d049_consumer_gate.py --gate 1|2|3|4|all --out <json>`，全 PASS exit 0、判据数值随 JSON 落盘（常量预注册在文件头）——训练发射前验证 vb 连续动作契约：**G1 契约一致性**（两种子 act 真采样 vb_action 不同 / mock train 循环同值贯穿：residual 48 维 action[45:48]==vb_action、plain 31 维 28:31 同验 / act logp 手算 aux+z+vb Normal 密度对拍 1e-5 / ksg 资格检查 buffer vb_action 重算 ratio≈1 + 篡改探针（vb 槽位喂 det 值必被检出=门有区分力）/ det 模式 vb_action==vb_logits 评测口径不变）；**G2 小环境信用分配**（toy bandit，reward=1−‖softmax(vb_action)·[0.3,0.5,1.2]−0.9‖²：主臂复用真实 update() 400 轮（lr=3e-3+vb_init_std=-1.0；首版 lr=1e-2 过冲震荡 FAIL 0.6394 带外已修，门内刻意不开 ksg=信用分配归因干净），判据=末 20 轮平均加权速度 0.667→0.9±0.15（owner 冻结带）；对照臂=缺陷版复现（categorical 簿记+确定性执行，期望梯度恒 0）3 种子 ≥2 不收敛，证明门能区分缺陷/修复）；**G3 单头超限回滚**（只对 vb 头注大梯度、其余 grad=None、lr=100：kl_vb>0.05 全回退后拒步 / joint==kl_vb（z=res=0）证 kl_vb 在联合目标 / 参数+Adam 状态逐位还原 / 对照小步 lr=3e-4 接受=守卫不过度触发）；**2026-09-12 本机实跑 --gate all 3/3 全绿 exit 0**（G1 7 检查、篡改探针 ratio 偏差 153 检出；G2 主臂 init=0.7007→final20=0.9075 正中带、对照臂 3/3 停滞 0.6667；G3 kl_vb=2.12e4 拒步、lr_scale 触底 2^-6、参数+Adam 状态逐位还原）；2026-09-11 登记，2026-09-12 返工实跑更新；**D051（§5k）增 G4 BC 目标正确性子门（--gate 4/all）**：vb_bc_loss 在随机 encoder 特征上 800 步拟合 obs[67] cmd→w*（合成 obs 噪声尺度 0.05 / lr 0.3 定档记录在文件头常量区），判据 9 格 |ŵ·v−proj_v(cmd)|≤0.05 + 零更新对照臂至少一格超差 | D049b / DS_CONTINUOUS_EXECUTION_PLAN §5j / §5k |
