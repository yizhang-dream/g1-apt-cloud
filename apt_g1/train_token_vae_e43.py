"""E43: fast-bin-weighted direction disentanglement for DirSpeedPhaseTokenVAE.

Per-bin leakage probe (probe_vae_e39_bins.py) showed E39's z still leaks
direction in the FAST speed-bin region (vb=2: fresh dir acc 0.771 vs majority
0.702, leak +0.069), while the slow region is clean (+0.014). The overall probe
(+0.019) was masked by the 57% clean slow rows. This explains E40's speed-
induced drift (straightness 0.89 at vx 0.456): pushing z fast enters the
residual-entanglement region.

E43 keeps the E39 architecture and both adversarial heads, but adds EXTRA
direction-squeezing pressure on fast-bin rows:
  adv_d = mean(CE_none(dir_head(z), db)) + fast_extra * mean(CE_none[...][vb==2])
with fast_extra = 2.0 (fast rows get 3x total pressure). Prediction: fast-region
leak -> ~0, and then the E40-style high incentive should deliver higher speed
WITH straightness kept.

Saved: outputs/token_vae_e43/{vae.pt, pca.npz, z_walk.npy, vbin_meta.json,
dbin_meta.json, meta.json}.
"""
...
class DirSpeedPhaseTokenVAE(nn.Module):
    """Identical architecture to train_token_vae_e39.DirSpeedPhaseTokenVAE."""
...
def main():
    ...
    adv_dir = 3.0
    adv_spd = 3.0
    fast_extra = 2.0  # extra direction-squeezing weight on fast-bin (vb==2) rows
    data_dir = "data/exp_all3"
    out_dir = "outputs/token_vae_e43"
    ...
    for ep in range(epochs):
        ...
        for xb, yb, pb, vbb, dbb in dl_tr:
            ...
            ce_none = ce(dir_head(z), dbb, weight=dcls_w, reduction="none")
            fast = (vbb == 2).float()
            adv_d = ce_none.mean() + fast_extra * (
                (ce_none * fast).sum() / (fast.sum() + 1e-6)
            )
    ...
    with open(os.path.join(out_dir, "meta.json"), "w") as f:
        json.dump({"window": window, "latent_dim": latent_dim, "hidden": hidden,
                   ...
                   "adv_dir": adv_dir, "adv_spd": adv_spd, "fast_extra": fast_extra,
