...
def build_args():
    ...
    # D056 netfwd 臂（DS_CONTINUOUS_EXECUTION_PLAN §5p）：净前向奖励
    # +w·clamp(net_vx, 0, vx_cap)/vx_cap（net_vx=机体线速度旋到初始航向系
    # 的前向分量，cos 因子自动惩罚偏航；yaw_rel 与 yaw-rew 块同源同符号；
    # vx_cap=2.0 cfg 复用不暴露 CLI）。默认 0 = 关（reward 路径/分解日志
    # 逐字节不变）。netfwd 臂配方 --net-vx-scale 2.0（§5p）
    ap.add_argument("--net-vx-scale", type=float, default=0.0,
                    help="D056: net-forward reward weight "
                         "+w*clamp(net_vx,0,vx_cap)/vx_cap (net_vx=body lin "
                         "vel rotated into the initial-heading frame, cos "
                         "factor penalizes yaw; vx_cap=2.0 reused from env "
                         "cfg; default 0 = off, byte-identical)")
...
def main():
    ...
    # D056：旗标默认零时回显逐字不变（>0 才显示）
    net_vx_cfg = (
        f" net_vx_scale={cli.net_vx_scale}" if cli.net_vx_scale > 0.0 else ""
    )
    ...
    print(
        ...
        f"{ksg_cfg}{pcap_cfg}{vb_cfg}{heading_cfg}{yaw_rew_cfg}{vx_min_cfg}{max_vx_cfg}{net_vx_cfg}",
        flush=True,
    )
    ...
    cfg.net_vx_scale = cli.net_vx_scale  # D056 netfwd：奖励项接线（默认 0=零变化；vx_cap=2.0 cfg 复用不暴露 CLI）
