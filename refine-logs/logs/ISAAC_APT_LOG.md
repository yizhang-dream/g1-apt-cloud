...
脚本 `_archive/train_perception_distill.py`。
...
3. 0.08 的结论修正为：**偏移通道在平坦无损、rough 0.06 基本持平、
   ...
   0.08 是两类路径的共同能力边界（对照先验 0/9 完成）。〔归因修正 2026-08-14，
   见 MQ09：此"能力边界"是蒸馏路径（无 planner 10Hz 重规划）的边界，非冻结
   解码器本身——同一解码器 + planner 重规划在 MuJoCo rough 0.08 walk≈flat。〕
   连续插值在 0.06 与离散路径持平，未见明显优势。
...
`training/train_token_vae_e27.py`、`isaac/token_window_vae.py`。
...
`e30_eval.log`、`apt_g1/viz/plot_latent_cmp.py`（E27–E30 对比图脚本，产出
...
**改动**：
- `_archive/train_token_vae_e31.py`：`SpeedPhaseTokenVAE`——decoder 输入加 v_bin embedding
...
代码：`_archive/train_token_vae_e31.py`、`token_window_vae.py`（SpeedPhaseTokenVAE）、
...
**改动**：
- `_archive/train_token_vae_e35.py`：`DirSpeedPhaseTokenVAE`——decoder 加 ψ_bin embedding
...
**问题**：早期阶段视频是 matplotlib 2D 骨架（`_archive/animate_skeleton.py`），因 Isaac Sim
的 GPU 渲染（viewport/hydra）在服务器段错误。用户指出骨架视频不真实。

**解决**：`viz/replay_render_mujoco.py`——把 Isaac rollout 的 npz（base 位姿 + 29 关节角，
...
之后每版实验按 `rollout_log_joints.py`（无相机）→ `viz/replay_render_mujoco.py`（3D 渲染）
...
代码：`_archive/train_token_vae_e35.py`、`token_window_vae.py`（DirSpeedPhaseTokenVAE）、
`apt_flat_env.py`（latent_dir_bins）、`train/eval/rollout_log_joints.py`
（--latent-dir-bins）、`viz/replay_render_mujoco.py`（MuJoCo 3D 渲染）。

### G6 相机冒烟：enable_cameras 段错误复现（2026-09-22）

`smoke_camera_depth_g6.py` 8envs 实跑段错误复现于
`omni.kit.widget.viewport` `texture.py:377` `__enable_hydra_engine` ←
`AppLauncher._create_app`（业务代码前），与 E29 render_walk 历史故障同因；
`enable_cameras` 在 lab-ts（isaaclab 2.1.0 / isaacsim 4.5.0）不可用。
stub JSON=`outputs/sync/g6/smoke_camera_depth_g6.json`。
