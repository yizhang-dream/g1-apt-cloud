...
- 2026-09-11 早：**D049 闭环（负结果，tracker D 66→67）**：§5j 两臂（a=D048i 配方逐字自然档 / b=同配方+策略 vb 软权重 48 维）联合门 8/8 全败。三机制证据：策略 w 未编码命令（分离度±0.01、恒定混档、force 下不动）；**解码器档位语义完好**（b 臂 force 谱 {0.003, 0.471, 1.239}≈吸引子+CEM 包络=问题在策略侧非表示层）；方向两臂全败 115-165°=方向轴成统一瓶颈。**G-1 判定修正：接口已建可复用，但选择权归策略≠命令响应自动出现**——论文 gait logit 的工作性依赖奖励结构/课程/预算（未拆解），差距未闭合。执行中两次发射前拦截（plain 架构违预注册残差单变量链 1457395 / force 电池降级违判读语义），代码链 86c1057→45fdcb0→1be8239。
