...
| 速度-连续可调性 | **D048q-Q1（正向闭合）** | vb 软混（嵌入线性混合）闭环电池：2 VAE×2 档对×α 三点全严格单调、近似端点线性插值（speedA pair12 1.197/0.977/0.712 vs 线性 1.10/0.89/0.68 略凸），noaux 同值=残差不主导，72/72 存活 | **连续可调（近似线性谱）**：速度轴非锁死三吸引子；speedA pair12 软混覆盖 0.71-1.20 连续带（含 0.6-0.8 命令带）→ 连续 vb 臂（D049）获得直接证据基础 |
| 方向轴 | D046b（token 层 vb/db 读出不可判读）+ D048m（方向=z 彩票）+ **D048q-Q2（db 闭环）** | db 八档闭环：canon lat 跨度 [−10.5,+8.3] 强信号但块状非 8 方位正弦环绕（db0=站立/v_thresh 低速桶）；speedA 信号弱一个量级且与 canon 不同相 | 方向不可控证据链闭合三层：z 彩票 + db token 层不可判读 + **db 闭环粗编码且随 VAE 重训不稳定**——方向控制不能靠 db 条件通道 |
