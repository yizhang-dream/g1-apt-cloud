...
5. 【选阈结果回填（2026-09-06，三探针完成后）】：t001(0.01) 冻死（rew
   钉地板 1.6392[250–299]、回滚 34.5% 超线、lr 压 30×、std 全冻
   0.01832→0.01834）；t003(0.03) 弱可学习（峰 1.7340@it157 持平 fix2
   峰、it200–299 均值 1.6679 地板上方、回滚 29.8% 贴线、lr 压 10× 至
   末值 2.24e-5）；t010(0.1) 动态最好（峰 1.7613@it150 三档最高、回滚
   19.3%、lr 温和压至 8.85e-5、唯一 std 上行 →0.01859 = v1 幅度 1/6）
   但 it200–249 起跌破地板（1.6272→1.6156）。**按本节预注册标准选定
   thr = 0.03**（唯一「最紧且可学者」）→ C2′ 已发射
   （`isaac_e49c2p_guard_s0`，[CFG] 核对一致）。两条警示随选阈交付：
   ① 29.8% 预算边缘 + lr 压 10×，学习裕度薄；② 三档 250–299 末段共同
   下行 + ev 全程 ≈0 = 阈值无关的窗口内退化信号（备选 0.05 插档/延长
   窗口，均未执行，归 owner）。共同模式（kl_mb it50 后钉 thr 下方贴顶
   平衡、kl_mb max=14.78 三档同[it0 守卫前]、fall 全程 0、clip 压至
   0.034–0.44）与全数字表见 tracker E.md E49-C2p-probe 行。
...
- **验证结果（2026-09-06 深夜）**：隔离目录两轮——旧代码 6/9（case7
  ...
  [1.47,2.57,4.98,5.19] = 参照切换直接证据。**已部署执行根**（2026-09-06
  深夜，C2 评测 14:09 跑完后换装；sync pull 8451f7f → cp 三件套 →
  sha256 三对逐位一致：ppo_core b1cd649b / train 78cc99ee /
  test 9556e61f）。
...
- 2026-09-06（C2 完成收账）：C2 训练+8 组评测全部完成（final 四组
  ...
  → P 探针 ×3 发射。
- 2026-09-06（v2 探针轮 + C2′ 发射）：三探针（t001 owner 亲发 14:17 /
  t003 watcher+owner 双发射[同 seed 同配置等价，最终数据取 owner 实例
  train_log.json，train.log 双写截断弃用] / t010 owner 亲发 14:31）全部
  完成并收数——判定与选阈 0.03 见 §B.3′-5 回填，Run 行 E49-C2p-probe
