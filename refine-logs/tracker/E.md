...
| E49-C2p-guard-s0 | **E49-C v2 流程第二步：单 seed 正式 run**（选定 thr=0.03，1000 it + final/btail 双 ckpt 评测；v2 守卫 = rollout 参照 + 拒绝步恢复参数与 Adam） | 128×1000，seed 0，`--diag-log --kl-guard 0.03`；产物 `isaac_e49c2p_guard_s0/` + `outputs/e49/eval_e49c2p_guard_s0_*.json` | 预注册 J1/J2/J3（§B.4，同 v1 判据口径） | 判读中 | **训练完成，判读（J3 基于 v2 正确判据量）**：J1（it800–999）：**rew 1.6295 / dxy 0.5544 / fall 0**——卡在站立地板 1.64 正下方、PASS 线 1.70 未达 → **FAIL（边缘）**，但远好于 fix2（1.0818）与 v1 C2（0.9798）= 退化被完全止住；J3(a) 超阈 **1225 = 回滚 1225 零差值**（v2 对账完美）；(b) **29.17%** 贴 30% 线；(c) 骤降 **0 次**（fix2 7 / v1 19 → 0，灾难步完全消除）；act_std 冻结 ≈0.0185、lr 全程压 1.5–3e-5、kl_mb it100 后钉 0.0204（thr 下方贴顶平衡）、clip 恒 0.188 健康、**ev 全程 ≈0（critic 从未学会）**；rew 峰 1.7495@it77 后即无上行（1000 it 几乎平线 1.62–1.68）。**final 评测（it1000）：det+sample × train+60s 全部 completed 零摔（h_min 0.755、vx≈0.003、disp≈0.09m 原地站立满时长），但 0.09m ≪ 5m/15m 行走线 → J2 final FAIL（0 行走格）**。best-tail = it850（[827,877) 窗均 1.6668），btail 评测进行中。**形态判读（§B.6 分支 2）**：非「慢漂移退化」而是「**稳定但被约束到不学**」——守卫完美实现信任域（灾难步清零、final 从 v1 的 3 秒摔变稳定站立 120s）但把学习一起锁死；`lr` 被压到 1.5e-5 无回升空间 = 守卫自适应 lr 的下限效应；**ev 全程 ≈0 = critic 从未学会是新关键观察**（与探针共同模式一致）——「KL 被压但 rew 停滞 → 查是否过紧」+「critic 早期 ev≈0」两路汇合，正式裁决归 owner |
