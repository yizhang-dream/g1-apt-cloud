...
| E49-A-fix-s0 | **修正训练器基线**（§3b 五缺陷修复后，lr 3e-4 不变 = 单变量；GAE done\|trunc 双切断+aux 剔除+真指标） | 128×1000，seed 0，`--token-mode --ppo-epochs 1` | 曲线 + A 60s×3（it_50 + final 双评） | DONE | **无单调崩坏但判据 FAIL**：rew 峰 1.79@it51 → 300–700 掉 0.86–0.96 → 700–800 回 1.55 → 末段 0.91（final 1.29 vs 原 0.62；原形态"单调崩"消失）；**真诊断读数：akl it10 即 ~10、全程 10–500，clip 恒 0.83，std 0.0183→0.0301（+64%），klp 224→1102（先验漂移 5×）**——修复改变动力学但未带来健康优化（无 E46 式 1.6→2.07 上升）；eval 数字见 `outputs/e49/eval_e49a_fix_s0_{it50,final}.json` |
| E49-A-fix-lr1e4-s0 | 降温单变量臂（owner 计划第③步；其余与 fix-s0 全同） | `--lr 1e-4` | 曲线 + akl/clip 量级（首要观察） | RUNNING（09-05 晚发射） | 已发射；判据：akl/clip 是否量级回落 + rew 是否持续上升 + final 不逊 best |
