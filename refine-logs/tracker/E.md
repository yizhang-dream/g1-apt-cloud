...
| E49-A-fix-lr1e4-s0 | 降温单变量臂（owner 计划第③步；其余与 fix-s0 全同） | `--lr 1e-4` | 曲线 + akl/clip 量级（首要观察）+ it_50/final 双评 | RUNNING（09-05 晚发射，~it250 时 rew 1.60→1.72 缓升） | **akl 量级回落 ~2 个数量级**（it50：0.047 vs 基线 22.7）、clip 0.42–0.53、std 钉在 0.0183–0.0187、klp 钉在 ~223（分布近乎冻结）；**但训练 env fwd 塌到 0.003–0.09 m/s（近站立）而 rew 仍 ~1.6**——"健康 PPO 统计 + 冻结分布"≠走路，暴露**训练奖励对"走 vs 站"区分度弱**的新问题（vel 项权重/σ 待查）；双评已挂服务器端 runner（训练完自动跑，产物 `eval_e49a_fix_lr1e4_s0_{it50,final}.json`） |
