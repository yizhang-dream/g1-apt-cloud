...
| E49-init-ctrl | **未训练初始化对照**（owner 评审要求的廉价对照；确定性初始策略 ≈ 固定官方均值 token 回放） | `--init-policy`，token-mode | A 60s×3 seeds | DONE | **3/3 零摔 14.2–14.4m / vx 0.37 / h_min 0.755**——冻结 SONIC 先验在初始化处已≈E45 终态水平（14m/0.27）；E49-A it_50 的 29m = 初始的 ~2× = 真实学习量；"搜索不是瓶颈"由此量化 |
| E49-A-fix-s0 | **修正训练器基线**（§3b 五缺陷修复后，lr 3e-4 不变 = 单变量；GAE done\|trunc 双切断+aux 剔除+真指标） | 128×1000，seed 0，`--token-mode --ppo-epochs 1` | 曲线 + A 60s×3（it_50 + final 双评） | DONE | **无单调崩坏但判据 FAIL**：rew 峰 1.79@it51 → 300–700 掉 0.86–0.96 → 700–800 回 1.55 → 末段 0.91（final 1.29 vs 原 0.62；原形态"单调崩"消失）；**eval：it_50 = 3/3 零摔 21.4–21.5m/0.34（原 E49-A it_50 为 29.0m/0.47）→ final = 3/3 零摔但 0.1–0.3m 塌到站立**（final≪best 判据 FAIL，与原 run 同病但无暴力形态）；**真诊断读数：akl it10 即 ~10、全程 10–500，clip 恒 0.83，std 0.0183→0.0301（+64%），klp 224→1102（先验漂移 5×）**——修复改变动力学但未带来健康优化（无 E46 式 1.6→2.07 上升） |
| E49-A-fix-lr1e4-s0 | 降温单变量臂（owner 计划第③步；其余与 fix-s0 全同） | `--lr 1e-4` | 曲线 + akl/clip 量级（首要观察）+ it_50/final 双评 | DONE | **负结果：lr 单变量失败**。akl 回落 ~2 个数量级（it50 0.047 vs 基线 22.7）、std 钉 0.0183–0.0197、klp 钉 219–223（分布冻结），**但 clip_frac 仍恒 0.42–0.55**（lr ÷3 裁剪率不落 → σ=e⁻⁴×64d 联合概率比对均值头超敏，裁剪高企非 lr 单因子）；训练 fwd 全程未起飞（850–999 段 0.00–0.15）而 rew 1.644 = **站立地板**（见 E49-rew-floor 行）；**双评 it_50 = final = 6/6 零摔跑满 3000 步纯站立（vx 0.000–0.001、disp 0.098–0.11m）**——比基线更早失去行走（fix-s0 it_50 走 21.4m）；iter999 loss 突跳 3628（此前 4–8）。慢更新+分布冻结 = 锁死初始站立模式，非保住先验行走 |
| E49-rew-floor | **奖励地板量化**（诊断轮首发现；不跑实验，cfg 期望值手算 + 与两臂实测对账） | `_get_rewards` 各项在 cmd~U[0,0.8]、vy=yaw=0 下的站立期望值 | 解析 + 实测吻合 | DONE | 站立期望 rew = E[exp(−cmd²/0.25)]≈**0.541** + yaw 0.5 + upright 0.1 + height 0.5 = **1.641**，与 lr1e4 臂实测 rew 1.644/fwd≈0.000 **精确吻合**；完美跟踪 0.8 期望 ≈2.07（stillness_vx 0.05×0.64≈0.03 扣除）。**走/站奖励差在平均命令下仅 ~0.43，低命令下趋零**——"训练 rew 仍 1.6"是站立地板不是学习信号；fix-s0 中段 0.86–0.96 **低于**地板（摔/复位态丢 upright+height），分项窗口记录才能解释（诊断步骤 ③ 的靶子） |
| E49-diag-sample | eval 采样模式冒烟（诊断轮：区分"探索扰动 vs 策略真退化"因子②的度量口） | `eval_apt_isaac.py` 新增 `--sample`（act 采样，与训练 rollout 同口径）；fix_s0 it_50 × A × 3 seeds | det vs sample 对照 | DONE | **it_50 上无差**：vx 差 ≤0.002（0.342 vs 0.342）、disp 低 1–2%（21.5→21.3m）、6/6 零摔——σ 钉在 e⁻⁴ 量级使采样≈确定性；**it_50 的 21m 是采样鲁棒的**，该因子区分力要到 std 已增长的 ckpt（fix-s0 中后段快照）上才可能显现 |
...
| E49-C2p-probe | **E49-C v2 流程第一步：三候选 schedule-fair 探针**（§B.3′ 文献量级 {0.01, 0.03, 0.1}；`--iters 1000 --probe-iters 300` 调度公平；owner 深夜审计后的重定阈程序） | 128×300（调度按 1000），seed 0，v2 守卫（rollout 参照+拒绝步恢复参数与 Adam）；产物 `isaac_e49c_p2_t{001,003,010}_s0/`（服务器） | §B.3′-3 选阈标准（rew 离地板上行 + 回滚 ≤30%，不接受冻死换稳定） | DONE | **t001(0.01) 冻死**：rew 钉地板（250–299 段 1.6392）、回滚 34.5% 超线、lr 压 30× 至 ~1e-5、std 完全冻结（0.01832→0.01834）、ev −0.004；**t003(0.03) 弱可学习**：it50–249 离地板上行、峰 1.7340@it157 持平 fix2 峰 1.731@it100、it200–299 均值 1.6679 地板上方、回滚 29.8% 贴线、lr 压 10× 至 2.24e-5、std 冻结 0.01839；**t010(0.1) 动态最好但末段崩**：峰 1.7613@it150 三档最高、回滚 19.3%、lr 温和压缩（末 8.85e-5）、唯一 std 上行（→0.01859，幅度仅 v1 的 1/6）、但 it200–249 起跌破地板（1.6272→1.6156）、clip 高 0.44。**共同模式（与阈值无关）**：三档 250–299 全下行、ev 全程 ≈0（critic 失同步）、kl_mb it50 后钉各自 thr 下方（贴顶平衡）、kl_mb max=14.78 三档同（it0 守卫介入前）、fall 全程 0。**选阈 = 0.03（预注册标准下唯一「最紧且可学者」）**，两条警示随行：① 29.8% 预算边缘 + lr 压 10× 学习裕度薄；② 末段共同衰减提示 300it 窗口内存在阈值无关退化（0.05 插档/延长窗口为 owner 备选）。**v2 机制生效实锤**：clip 从 v1 轮 0.833 压至 0.034–0.44、akl 至文献量级、std 膨胀被抑制。防撞车记档：t001/t010 owner 亲发；t003 双发射（watcher 14:23 + owner 14:25 同目录并行至完成），最终数据取 owner 实例 train_log.json（同 seed 同配置等价），train.log 双写截断不可信、检查点两实例混存 |
| E49-C2p-guard-s0 | **E49-C v2 流程第二步：单 seed 正式 run**（选定 thr=0.03，1000 it + final/btail 双 ckpt 评测；v2 守卫 = rollout 参照 + 拒绝步恢复参数与 Adam） | 128×1000，seed 0，`--diag-log --kl-guard 0.03`；产物 `isaac_e49c2p_guard_s0/` + `outputs/e49/eval_e49c2p_guard_s0_{final,btail}_{train,60s}_{det,smp}.json`（8 组） | 预注册 J1/J2/J3（§B.4，同 v1 判据口径） | DONE | **J1 FAIL / J2 FAIL / J3(a)(b)(c) 全 PASS → §B.6 分支 2 后半（KL 已被压但 rew 停滞 → 查过紧）+ 新机制嫌疑，H-C 判读 =「信任域成立、学习被锁死」**。J1（it800–999）：rew **1.6295** / dxy 0.5544 / fall 0——rew 落 PARTIAL 带 [1.60,1.70) 但 **dxy 0.5544 < 0.65 → FAIL**；对照 fix2 1.0818 / v1 C2 0.9798 = 退化完全止住但站在地板下。J2：**final（it1000）12/12 rollout 零摔满时长站立**（h_min 0.755、vx≈0.003、disp≈0.09m ≪ 5m/15m 线，0 行走格，det+sample×train+60s 四组一致）——对比 v1 final 3 秒内 24/24 全摔 = 质变；**btail（it850）36/36 零摔 0 行走格**（唯二准行走：train_smp noaux seed3 4.611m 距 5m 线最近、60s_det aux seed1 12.506m 距 15m 线最近，均未达线；60s_det 中 aux/noaux det 模式部分重合属预期、seed1 分叉待查）。J3（v2 正确判据量）：(a) 超阈 **1225 = 回滚 1225 零差值**；(b) **29.17%** 贴 30% 线；(c) 骤降 **0 次**（fix2 7 / v1 19 → 0 = 灾难步清零）。机制量：rew 峰 1.7495@it77 后 1000 it 平线 1.62–1.68；std 冻结 ≈0.0185；lr 全程压 1.5–3e-5 无回升；kl_mb it100 后钉 0.0204（thr 下贴顶平衡）；clip 恒 0.188 健康；**ev 全程 ≈0（四段 −0.010～−0.003，critic 从未学会）vs fix2 后期 0.84**。**判读**：守卫机制侧全绿（对账/无灾难步/终态稳定），但 thr=0.03 下 29% 回滚率把策略与 critic 的有效学习率一起锁死（lr 被自适应压至 1.5e-5 量级）→ 「稳定但被约束到不学」。**新机制嫌疑（判别价值高，归 owner）**：v2 拒绝步恢复**全模型 state_dict 含 critic**——29% 步被高频倒带可能是 ev 全程 ≈0 的直接原因（t010 lr 8.85e-5 ev 仍 ≈0 排除纯 lr 因子）；判别实验 = 守卫只回滚 policy 头不动 critic（单变量）。下一干预候选（全部涉冻结面，归 owner 裁决）：① thr 上探 0.05/0.1 长窗（t010 动态最好）；② 回滚范围单变量（policy-only vs 全模型，直指 ev≈0）；③ critic lr 与守卫解耦 |
...
5. **修正轮结果（09-05 晚，§3b 修复 + 初始化对照 + 修正基线 + 降温臂）**：
   ① 初始化对照（E49-init-ctrl 行）：未训练确定性策略 3/3 零摔 14.2–14.4m/0.37
   ——冻结 SONIC 先验自带 ≈E45 终态水平的行走；"it_50 即 29m"中真实学习增量
   = 初始的 ~2×，A 问结论（能学会、搜索不是瓶颈）成立但表述须带初始底座。
   ② 修正基线（E49-A-fix-s0 行）：GAE/aux 修复改变了崩坏形态（单调崩消失、
   大部分区间零摔），但判据 FAIL——rew 无持续上升，且**真新旧 approx_kl
   it10 即 ~10、全程 10–500，clip_frac 恒 0.83**（教科书健康量级 0.01/0.1–0.3；
   机制上与 σ=e⁻⁴×64 维一致：均值头一个 Adam 步即可把联合概率比推出 clip
   窗）。**这构成"每次更新过大"假设的第一次正确证据**（原 kl med 374–654
   的证据无效，见结论 2 修订）——但"过大"与"崩坏"的因果链仍未闭：300–700
   段 rew 掉到 0.86–0.96 时 akl 同段反而回落，700–800 回弹与 klp 5× 漂移并存。
   ③ std 0.0183→0.0301 单调抬升 + klp 224→1102 = 分布在持续离开官方 token
   标定邻域（终态 fwd 0.06 衰向站立与 tanh 臂同形）。④ 下一步 = lr 1e-4
   单变量臂（已发射）→ 若 akl 量级仍不落，候选 = KL 早停 / log_std 起点与
   熵系数整体重标（64 维 × 小 σ 的参数化本身可能是病根，需 owner 裁决）。
6. **诊断转向（09-05 深夜 owner 决策：暂缓奖励调参与 E49-B，先做"训练—评测
   一致性诊断"）**：① lr1e4 臂负结果（双评全站立）宣告降温单变量失败，
   clip 恒 0.42–0.55 证明裁剪高企非 lr 单因子；② 奖励地板量化（E49-rew-floor
   行）把"奖励对走/站区分度弱"从候选解释升级为有解析+实测对账证据的主嫌
   ——但 owner 纪律保持：**先用已有轨迹验证候选奖励能正确排序才能重标**，
   且不得同时加权重+收窄 σ²（收窄可能让当前可达速度区无奖励）；③ 三处
   判断收紧：clip_frac 0.42–0.53 + KL 为小批次统计 ⇒ "PPO 完全健康"证据
   不足；std/kl_prior 不变推不出"分布冻结"（均值可移）；奖励窗口不一致
   （rew=全 rollout 均值 vs fwd=iter 末瞬时）⇒ 奖励不区分走站仍待分项
   验证；④ timeout 修复按 owner 语义落码（E49-trainer-fix2 行）+ 训练器
   版本冻结；⑤ **E49-B 启动条件改**：不必等 A 完全成功，但须训练器与
   评测可信、配方冻结——若 A 在修正配方下仍稳定退化，B 恰好检验显式
   相位是否有帮助；无限等待"稳定配方"会让归因实验永远无法开始；⑥ 之后
...
9. **E49-C v2 轮判读（09-06，三探针→选阈 0.03→C2′ 1000 it + 8 组评测
   全链完成；判据 = §B.3′/§B.4，H-C 最终判读）**：① **守卫机制侧全绿**
   ——J3(a) 超阈 1225 = 回滚 1225（v2 正确判据量下仍完美对账）、
   J3(c) 骤降 0 次（fix2 7 / v1 19 → 0 = 灾难步清零）、final 终态从
   v1 的 3 秒全摔变为 **12/12 零摔满时长稳定站立**（h_min 0.755、
   disp≈0.09m）——信任域本身被证明可实现且有效；② **但学习被锁死**
   ——J1 FAIL（rew 1.6295 落 PARTIAL 带、dxy 0.5544<0.65）、J2 FAIL
   （final/btail 0 行走格，36+12 rollout 全站零摔）、rew 峰 1.7495@it77
   后 1000 it 平线；机制量三条互洽：回滚率 29.17% 贴 30% 线、lr 被自适应
   压至 1.5e-5 无回升、std 冻结 0.0185——「稳定但被约束到不学」，与
   探针三档的形态谱系（0.01 冻死 / 0.03 贴线 / 0.1 动态最好但末段破地板）
   连成完整证据链：**在 64 维小 σ 直出 token 体系下，文献量级 KL 阈
   （0.01–0.1）与可学习性存在结构性张力**；③ **新机制嫌疑（本轮最有
   判别价值的产出）**：**ev 全程 ≈0（critic 从未学会）vs fix2 后期
   0.84**——v2 拒绝步恢复全模型 state_dict 含 critic，29% 高频倒带是
   ev≈0 的头号候选解释（t010 lr 8.85e-5 ev 仍 ≈0 排除纯 lr 因子）；
   判别实验 = 回滚范围单变量（policy-only vs 全模型）；④ 嫌疑队列现状
   （与 §A′-7 合并）：累计更新偏离（守卫已按设计压制但学习代价过大）
   > 探索方差单调涨（守卫 run 中 std 冻结 = 该嫌疑被守卫间接压制，
   退化也随之消失——**反证 σ 膨胀与退化的因果关联增强**）> critic
   ev≈0（新实证）> 奖励设计（冻结）；⑤ **下一干预候选（全部涉冻结面，
   归 owner 裁决）**：a) thr 上探 0.05/0.1 长窗；b) 回滚范围单变量
   （policy-only，直指 ev≈0）；c) critic lr 与守卫解耦；d) 接受
   「守卫+低 lr」为稳定配方基线、把问题转回奖励结构（走/站差弱）。
   H-C 最终表述：**KL 信任域守卫可实现且能完全消除退化与灾难步
   （机制成立），但在当前 64 维直出 token 配方下以锁死学习为代价
   （充分性不成立）**——E49-C 两轮（v1 不充分 / v2 锁死）合起来
   把「退化-学习」trade-off 曲线的两端都测到了。
