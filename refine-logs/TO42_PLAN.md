...
- **执行身份修订 v4（2026-09-04 owner 规模指令「最起码几千个 env，论文里
  ...
  - 预算估：探针 ~¥0.5 + 全链（4×500it 串行 + 流水评测）~3.5–4.5h ≈
    ¥25–45（L20 单价待资源单核实）。
- **执行身份修订 v5（2026-09-04 上午 owner 决断链「改成1024立刻开始」）**：
  修订 v4 的 L20 路线被**资源饥饿阻断**（鉴别诊断：同号新镜像同项目，A10
  2 分钟入运行 vs L20 探针排队 8h+ 零调度，updateTime 冻结于创建时刻——
  Argo pod pending，非账号/配置问题，CLI 无队列位置查询）。操作点改
  **1024 envs × 500 it × minibatch 4096 @ A10 24G**（样本预算 = 128×2000
  的 2×；24×1024/4096 = 6 minibatch/epoch 整除），其余配方逐字不动；训练
  并发 1（串行配对）。**A10 探针 PASS（TASK_20260904_023，10:5x）：峰值
  3283 MB（远低于 22G 判据）、dt≈2.6–3.5 s/iter、rc=0**——此前 15MB/env
  的估算模型严重高估，实测边际显存极小（此遥测为后续若回 2048 的依据：
  2048 估 ~4–6G 亦容于 A10，dt 约 ×2，另立 lineage 再议）。全链
  TASK_20260904_025（`--profile a10_1024`）10:5x 发车，守望器 5 min 快照
  日志（gz+b64 receipt 通道 + 增量原子 bundle）。与 lab-ts 128 线
  （independent lineage）永不合并；判据沿用修订 v4 重述（配对 contrast
  主表，TO41 前沿 descriptive）。预估全程 ~1.8–2.2h。
