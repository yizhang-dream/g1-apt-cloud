...
- **2026-09-05（五）owner 诊断转向决策 + 训练器修复二轮 + 奖励地板量化**：
  ① **暂缓奖励调参与 E49-B 点火**——继续发射长训练新信息有限，项目缺的
  是"RL 稳定增加了什么能力"的证据（初始化能走 / 早期快照更好 / 后期站立
  三者不能合并成"搜索不是瓶颈"或"直出已解决"）；转向**训练—评测一致性
  诊断四步**（收齐双评并核对命令/初态/映射/重置/时长 → 不训练 2×2×3
  短对照 → 窗口一致分项记录+整轮 KL → timeout 修复+配方冻结后的有限预算
  验证），执行回写见 E49_STATUS §3c。② 三处判断收紧：clip_frac
  0.42–0.53 ⇒ "PPO 完全健康"证据不足（且日志 KL 为小批次统计）；std/
  kl_prior 不变推不出"分布冻结"（均值可移）；"奖励不区分走站"降为候选
  （rew 全 rollout 均值 vs fwd 末时刻均值窗口不一致）——当晚 **rew-floor
  量化**（站立期望 rew=1.641 与 lr1e4 实测 1.644 精确吻合，完美跟踪
  ≈2.07）将其升为有对账证据的主嫌，但重标前置纪律不变：**先用已有轨迹
  验证候选奖励能正确排序，不同时加权重+收窄 σ²**。③ lr1e4 臂负结果
  收账（双评全站立、clip 恒 0.42–0.55 ⇒ 裁剪高企非 lr 单因子）。④
  timeout 终末价值语义 owner 钉死并落码：**复位前终末状态对应的价值**
  （`_reset_idx` 前抓终末 obs），非 rsl_rl 式下一局观测补偿；属训练器
  正确性修复非超参；post_update_kl（整轮更新后整批统一测量）同轮落码，
  **训练器版本自此冻结**，旧 run（fix-s0/lr1e4）带 trunc 自举=0 偏差不
  与新 run 严格可比。⑤ **E49-B 启动条件改**：不必等 A 完全成功，但须
  训练器与评测可信、配方冻结；若 A 在修正配方下仍稳定退化，B 恰好检验
  显式相位是否有帮助；无限等待"稳定配方"会让归因实验永远无法开始。
  ⑥ A/B 比较须补**相同修正训练器的 VAE 参照**；E45–E47 = 历史结果不
  承担新归因；每路线比较自己的初始化/固定预算终态/预定规则快照，区分
  训练种子与评测种子。⑦ 资源裁定：冻结 SONIC 主线继续；D042 只推进
...
- **2026-09-06 B4-lite 构成升级（owner 五点意见，协议另立）**：面向论文的
  动作清单与划分协议定稿 = `DS_B4LITE_ACTION_SPLIT_PLAN.md`（五维语义标签 /
  三层清洗——L3 执行失败保留为能力边界记录不清洗 / T-seg·T-fam·T-pair 三
  测试划分 + 演员分组 / 可学习表示参与声明 / 100–200 条独立原始序列 ×
  8–12 语义动作族 + 三集合预冻结 + 四问 manifest）。**§3 B4-lite 行的
  「每核心类 20–30 段 + 10 过渡段」构成以新协议为准**；B3' 门判据、B4 全量
  编码、T1 切分与其余条款不变。B4-lite 构建执行时开 Run 行 D045 起。
