...
> 执行事实源：`tracker/D.md`（Run 行只进那里）｜状态：**执行中（09-05 起
> 受 `DS_TERRAIN_ADAPTER_CHARTER.md` 纲领收编：Phase 1 全网格挂起、
> Phase 5 升级过渡器、G2/G3 降辅助指标——见 §11 修订记录末条；Phase 0
> 已 PASS = D034 + D035；B 线 D036–D038 链路全通）**
...
- **09-05 纲领收编（owner 两轮方向讨论定稿 = `DS_TERRAIN_ADAPTER_CHARTER.md`；
  本计划与其冲突的条款自即日起按纲领 §9 处置）**：
  ①**北极星改写**——主目标改为「冻结底座 × 共享地形适配 × 未见动作迁移」
  （纲领 §1），§0 北极星中「七地形中档可通行域 + 全速度段 0.2–3.0」降为
  测量维度与辅助指标（G2/G3 不删除、改判据表述）；
  ②**Phase 1 全网格挂起**——纲领阶段一改为**分层小集合先行**（行走/跑步/
  减速/自然过渡相关样本；B 线官方数据为主、A 线回路按需补；按原始序列和
  演员划分防相邻帧泄漏），网格重启须先按纲领 §8 修订过渡采法（补非 idle
  直切 / 变进入速度 / 变接触状态三类——原 A→idle→B 36 段只教停顿切换）；
  ③**Phase 5 升级**——从「4-way 选择器两臂对照」升为纲领阶段三：过渡器
  三任务（何时可切 / 过渡时间-时长-短时序列 / 执行后重规划）+ 对照加
  直接条件策略（共三臂起）+ 任务收窄为「平地↔踏石」与「低障碍前步态
  调整」两类 + 理想高度图先行再渐进感知噪声/延迟；§5 最小迁移实验
  （留出整个动作族测地形适配迁移）为主假设立项门，协议另立；
  ④**Phase 4 定位调整**——RL 底板训练不再是阶段终点，改为纲领阶段二
  「学习策略输出」载体与阶段三底座；双流形对照 VAE-S/VAE-L 保留为
  「先修表示」分支工具，其结论表述受纲领 §7 误判防线 2 约束
  （VAE-L 无提升 ≠ decoder 容量耗尽）；
  ⑤新增**阶段四（解冻裁决）**不在本计划范围，届时按纲领 §6 三对照 +
  四分支另立协议；
  ⑥维持不变：Phase 0 结论（D034/D035）、§1 已冻结技术决策、§7B B 线门
  （B3 门转任纲领阶段一质检门）、mjlab 地形基准、卫生学铁律、Run 行
  纪律。

- **09-05b A 线退役（owner 指令「直接放弃我们的采样数据，直接选用官方
  数据」；执行协议 = `DS_OFFICIAL_DATA_PLAN.md`）**：A 线 Phase 1 采集
  网格由「挂起」改**「取消」**——自采数据退出训练管线；§0.5 分工铁律
  修订：B 线升为**唯一数据源**（原「任何情况下不替代 A 线」作废），A 线
  既有资产（exp_all/ds_smoke/键位标定/命令标注）转为①代理标注校准源
  ②oracle 探针材料③潜在双流形对照臂——均非训练用途（owner 裁定点
  D3）；§3 过渡段采法由「采集设计」转为「受控过渡评测集协议 + 官方
  自然过渡切分的对照参照」（纲领 §8 三类非停顿样本改由官方 288h 连续流
  供给，T1 路径）；Phase 2/3 质量门、VAE 训练架构、held-out 检验机制
  全部保留，数据入口换官方 B4 npz，标签来源换代理标注（transl 差分 +
  类目，A1）。
