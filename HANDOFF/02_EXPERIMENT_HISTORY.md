...
- 关键负结果：0.4 直走未攻克（task_success 0/162）——速度双向不跟踪（0.4 过冲 0.62 / 0.6 欠速 0.45）、横漂命令上界失稳（0.6 全域 5.8–6.4m）、早期跟踪增益被守卫同压（it50 vx_rmse 0.118→0.246）。
- 两处口径资产：带符号残差统计 + ckpt 身份信封必填硬化。
- 阶段1 正式关闭：交付 = 稳定训练基线 + 受控负结果三边界（晚期崩坏=更新过大已证已修 / 速度=双向不跟踪 / 横漂=命令上界失稳）；B5-gait 另行裁定、阶段2 不进入。
- 详 `refine-logs/tracker/D.md` D048f–D048k 行与 `refine-logs/DS_CONTINUOUS_EXECUTION_PLAN.md` §4–§5e。
